...
def publicar_evento_inventario(pedido_id: int, email: str, estado_inventario: str):
...
def procesar_mensaje(ch, method, properties, body):
    ...
    # Extraemos los datos del evento (Asumimos que S1 y S2 los incluyeron)
    pedido_id = mensaje.get("pedido_id")
    email = mensaje.get("email")
    producto_nombre = mensaje.get("producto", "Laptop")
    cantidad_requerida = mensaje.get("cantidad", 1)
    ...
    # 3. Publicar el evento para el Servicio 4 (Notificaciones)
    publicar_evento_inventario(pedido_id, email, estado_inventario)
